---
name: tailnet
description: Manages Hunter's home tailnet servers (brewserver, hp-server, micro-server) for moving files onto them and sharing local files off this Mac. Covers four actions - transfer a file/folder to a server over rsync+ssh, serve a local file or directory over HTTP on this Mac's tailscale IP so another device on the tailnet can open it, stop servers started that way, and send a file straight to a phone or tablet via Taildrop. Use when Hunter says things like "transfer this to my brewserver", "send this to code-projects on brewserver", "copy this over to micro-server", "serve this on my tailscale ip so I can read it on my phone", "let me pull this up on my iPad", "shut down the server", "stop serving that", or "send this to my iphone" (Taildrop, not AirDrop).
---
...
- SSH aliases for the servers already configured in `~/.ssh/config` (`brewserver`, `hp-server`, `micro-server`). If an alias doesn't resolve, that's a local SSH config problem, not something this skill fixes.
- `python3` and standard Unix tools only — no extra dependencies.

## Server registry

Read `servers.json` (in this skill's directory) before acting on `transfer` or `send`. It holds, per server: `ip`, `ssh_alias`, `user`, optional `default_dests` (named shortcut paths), and free-text `notes`.

| Server | IP | Alias | Notes |
|---|---|---|---|
| brewserver | 100.77.32.30 | `brewserver` | GTX 1050 Ti, CUDA runs. Dests: school -> `/mnt/School`, code -> `/mnt/code-projects`, brewmint -> `/mnt/BrewMint` |
| hp-server | 100.110.7.79 | `hp-server` | no default dests registered |
| micro-server | 100.108.167.49 | `micro-server` | Hermes agent, AI workloads |

If Hunter names a server or IP not in `servers.json`, ask him for the SSH alias rather than guessing one.
...
1. Resolve `<server>` to its `ssh_alias` and `user` from `servers.json`.
